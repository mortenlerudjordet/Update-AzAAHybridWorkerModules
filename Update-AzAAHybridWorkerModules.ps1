...
PREREQUISITES:
            ...
            Runs either with AzureRM og Az module, make sure that one of the modules are installed manually on the hybrid worker before running.
...
.Notes
            NAME:       Update-AzAAHybridWorkerModules
...
try
{
    ...
    if ((Test-Path Cert:\CurrentUser\My\$($AzureRunAsConnection.CertificateThumbprint)) -eq $false)
    {
    ...
    $cert = Get-ChildItem -Path Cert:\CurrentUser\my | Where-Object {$_.Thumbprint -eq $($AzureRunAsConnection.CertificateThumbprint)}
    ...
    $Null = Add-AzureRMAccount `
        ...
        -CertificateThumbprint $AzureRunAsConnection.CertificateThumbprint -ErrorAction Continue -ErrorVariable oErr
    if($oErr)
    {
        Write-Error -Message "Failed to connect to Azure" -ErrorAction Stop
    }
    $Subscription = Select-AzureRMSubscription -SubscriptionId $AzureRunAsConnection.SubscriptionID -ErrorAction Continue -ErrorVariable oErr
    ...
    {
        if (($AAworkerGroup.Name -ne $CurrentWorkerGroup.Name) -and (-not $UpdateAllHybridGroups))
        {
